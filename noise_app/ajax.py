...
@dajaxice_register
def loadComplaints(req):
  collection = db['noise_app_complaint']
  # make sure to change cursor into actual results
  aux = collection.find({})
  json_docs = []
  for doc in aux:
    json_doc = simplejson.dumps(doc, default=json_util.default)
    json_docs.append(json_doc)
  return simplejson.dumps(json_docs)
  
	
@dajaxice_register
def getOneAudio(req, id):
  collection = db['noise_app_audio']
  aux = collection.find_one({'_id': id}, {'_id': 1, 'start': 1, 'end': 1, 'lat': 1, 'lng': 1, 'db_max': 1, 'db_min': 1})
  return simplejson.dumps(aux)

@dajaxice_register
def getAudioDataRange(req, id, samplerate, request_start, request_end):
  collection = db['noise_app_audio']
...
#connect(0)
#print get_db(0, 0, "ds", 1328054400, 1328054400+60*60*24*30*12)
